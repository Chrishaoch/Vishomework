...
var width = document.body.clientWidth,
    height = d3.max([document.body.clientHeight-540, 340]);
...
function box(x,y,haonum,haocolor)//x, y is the center of the boxplot
{	var scale=60/haonum[4];
    var hao0=haonum[0]*scale,hao1=haonum[1]*scale,hao2=haonum[2]*scale,
	hao3=haonum[3]*scale,hao4=haonum[4]*scale;//hao2 is y

	svg.append("rect") // rect down 0.25
	   .attr("id","rect1")
	   .attr("height", hao2-hao1)
	   .attr("width", 6)
	   .attr("x", x-3)
	   .attr("y", y)  
	   .style("stroke", haocolor) 
	   .attr("fill", "none");
	   
	svg.append("rect") // rect up 0.75
	   .attr("id","rect")
	   .attr("height", hao3-hao2)
	   .attr("width", 6)
	   .attr("x", x-3)
	   .attr("y", y+hao2-hao3)
	   .style("stroke", haocolor)
...
{	
	var i=0,j=new Array(), haoz=[],z=[];	
	for (var k=0; k<12; k++) j[k]=new Array(9);
	for (var k=0; k<32; k++) z[k]=new Array(12);
	for (var k1=0; k1<32; k1++){for (var k=0; k<12; k++) z[k1][k]=[0,0,0,0,0];}
	 ...
	 {	
		...
		if(i===2) i++;
		var name=dims[i];
		for (var k=0; k<9; k++) {j[0][k]=data[k][name]; }
		for (var k=0; k<9; k++) {j[1][k]=data[k+9][name]; }
		for (var k=0; k<9; k++) {j[2][k]=data[k+18][name]; }
		for (var k=0; k<9; k++) {j[3][k]=data[k+27][name]; }
		for (var k=0; k<9; k++) {j[4][k]=data[k+36][name]; }
		for (var k=0; k<9; k++) {j[5][k]=data[k+45][name]; }
	
		for (var k=0; k<9; k++) {j[6][k]=data[k+54][name]; }
		for (var k=0; k<9; k++) {j[7][k]=data[k+63][name]; }
		for (var k=0; k<9; k++) {j[8][k]=data[k+72][name]; }
		for (var k=0; k<9; k++) {j[9][k]=data[k+81][name]; }
		for (var k=0; k<9; k++) {j[10][k]=data[k+90][name]; }
		for (var k=0; k<9; k++) {j[11][k]=data[k+99][name]; }
	
		for (var t=0; t<12; t++) 
		{
			haoz[t]=j[t].sort(function(a, b){return a - b});
			z[i][t][0]=haoz[t][1];
			z[i][t][1]=haoz[t][2];
			z[i][t][2]=haoz[t][4];
			z[i][t][3]=haoz[t][6];
...
var max = 5; 
  ...
  g.append("svg:g")
      ...
      .each(function(d) { d3.select(this).call(axis.scale(yscale[d]).tickFormat(function(d){if(d>=1) return formatter(d); else return logFormatter(d);} ).tickValues([0.001,0.01,1,10, 70])); })   // 
	.append("svg:text")
    ...
    .each(function(d,i) 
	{
	   ...
	   for (var ii=0; ii<32; ii++) {
	   	...
	   	box(ii*w/32+jj*6-w/33/2,h-z[ii][jj][2]-50,z[ii][jj],haocolors[jj]);
	   }};
  ...
  g.append("svg:g")
      .attr("class", "brush")
      //.each(function(d) { d3.select(this).call(yscale[d].brush = d3.svg.brush().y(yscale[d]).on("brush", brush)); })//-Hao 
	.selectAll("rect")
